...
# @jit(nopython=True)
def calc_dist(tour):
    a = np.sum([dist_matrix[i, j] for i, j in zip([tour[0:-1]], tour[1:])])
    return a
...
def two_opt(tour, dist_matrix, verbose=False):
    ...
    for swap_option in combinations(range(len(tour)), 2):
        ...
        tour2 = (
            tour.copy()
        )  # maybe instead of copying whole tour, I should just swap back items if dont accept change
...
def solve_it(input_data, num_starts=1, verbose=False, debug=False):
    ...
    pd.DataFrame(distance_matrix).to_csv("dist_matrix_py.csv", index=False)
    ...
    for start in tqdm(starts):
        ...
        print(f"initial greedy tour: {tour}")
